...
Avec ses caractéristiques naturelles, l'anse du Cul-de-Loup se singularise aussi par une activité industrielle de culture ostréicole aujourd'hui fortement présente (Fig. \ref{fig:historique-adcl}). La technique de culture très largement utilisée est basée sur de petites tables métalliques sur lesquelles sont disposées les poches d'huîtres (Fig. \ref{fig:table-adcl}).

\begin{figure}[!h]
    \centering
    \includegraphics[width=0.7\linewidth]{../images/table-huitre-02}
    \caption[Table à huître]{La production ostréicole présente dans l'anse du Cul-de-Loup utilise essentiellement une technique basée sur des tables.}
    \label{fig:table-adcl}
\end{figure}

En même temps que des tables de production sont installées au sein de différentes concessions, ces dernières peuvent faire l'objet de délimitation à l'aide de murets composés d'amoncellement de roches (Fig. \ref{fig:table-adcl}). La mise en place à la fois des tables ostréicoles et de linéaires rocheux n'est pas sans conséquences sur l'hydrodynamique de la zone. Les courants sont freinés, voir déviés, engendrant d'importantes modifications sur la répartition des particules sédimentaires formant la couverture de surface. Par voie de conséquence, la flore et la faune benthique directement inféodées aux cortèges sédimentaires, voient leur habitat modifié/perturbée, ce qui peut amener à la modification des espèces présentes (en nombre et en biomasse), voir dans les cas extrêmes à leur disparition.

Ainsi, une connaissance fine de l'hydrodynamique d'un lieu tel que l'anse du Cul-de-Loup, est une information de premier plan dans l'objectif d'une gestion durable et respectueuse des enjeux à la fois industriels et environnementaux de la zone.
...
Sur le plan de sa mise en \oe{}uvre, les calculs CROCO peuvent s’exécuter en mode parallèle, ce qui permet une grande souplesse d'adaptation entre temps nécessaire de calcul d'un côté et résolution et temps modélisé de l'autre.

Le système d'équations résolu par le modèle est rendu flexible par le biais de mots clés dédiés qui offrent la possibilité de choisir la configuration physico-chimique de l'eau souhaitée et adaptée au cas étudié.
La physique modélisée s'enrichit régulièrement de nouvelles adaptations qui prennent en compte des phénomènes toujours plus complexes avec une finesse croissante.
...
>>>>>>> 1270256979d6ea58085c59011b90dda7e559cc24
