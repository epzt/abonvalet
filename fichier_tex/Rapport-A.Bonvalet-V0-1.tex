...
Le maillage horizontal indiquant la position géographique des variables modélisées est contenu dans un unique fichier NetCDF.
On appelle le contenu de ce fichier la grille CROCO.
Cette grille est localisée en longitude et latitude à la surface de la Terre.
Elle contient trois maillages dont deux son issus de la translation Nord-Sud et Est-Ouest du maillage central sur une longueur d'une demi-maille.
Pour chaque maille de chacun des trois maillages, une latitude et une longitude sont attribués dans le fichier.
La grille CROCO contient aussi une donnée de bathymétrie / topographie pour chacune de ses mailles.
Enfin, elle contient un masque définissant quelles mailles sont définies comme toujours émergées.
Les équations du modèle ne seront ainsi jamais résolues au niveau de ces mailles.

Les résolutions verticales des trois maillages sont identiques.
Cette résolution verticale n'est toutefois pas définies dans la grille CROCO.
Elle est contenue dans les données de forçage initiaux et aux bords de la grille et est définie et indiquée au modèle dans les fichiers d'entrées \textit{param.h} et \textit{croco.in}.
...
Selon leur type, les variables physico-chimiques que le modèle propage sont définies sur l'un des trois maillages.
La table \ref{structure_maille horizontale} issue de \cite{grid_doc} illustre les trois positions, appelées ici U, V et H, que peuvent avoir ces variables relativement à une cellule du maillage central.
...
\begin{table}[h!]
    ...
    \end{tabular}
    \caption{
        Représentation d'une maille $i,j$ horizontale de la grille CROCO.
        ...
        L'existence des paramètres $V_{i,j+1}$ et $U_{i+1,j}$ suppose que la maille $i,j$ ne se situe pas à l'extrémité d'une ligne ou d'une colonne de la grille CROCO. Cette représentation est issue de \cite{grid_doc}
    }
...
D'autres encore sont exprimés sur toutes l'étendue horizontale de la grille mais sur une unique maille d'un point de vue vertical, ces données sont localisées soit en surface (ex. vent) soit au fond (ex. rugosité).
...
La seconde catégorie correspond aux forçages qui définissent les conditions aux bords latéraux de la grille sur toute la durée temporelle de la modélisation.
Le fichier correspondant, \textit{croco\_bry.nc}, contient notamment les données de température, salinité et éventuellement hauteur d'eau et vitesses des courants aux bords de la grille durant le temps de la modélisation.
...
Toutes les grilles contenues dans la grille principale extraient leur conditions aux limites à partir des résultats de la grille qui les précède.
Cette méthode d'emboîtement de grilles permet de s'affranchir du besoin de données externes pour les conditions aux bords de la grille d'intérêt la plus petite et résolue tout en maintenant un coût de modélisation assez léger.
...
\begin{itemize}
    \item le premier type d'emboîtement des grilles (AGRIF) est simultané et rétrospectif.
    ...
    Ce système d'emboîtement simplifie la modélisation du côté de l'utilisateur·ice qui n'a besoin de mettre en place et de démarrer le modèle qu'une unique fois pour obtenir les résultats de haute résolution des grilles filles.
    ...
    \item le second type d'emboîtement est linéaire, c'est à dire qu'une grille fille est toujours générées après l'exécution de la modélisation CROCO sur sa grille mère.
    Les données de sortie du précédent modèle sont utilisées par l'utilisateur·ice pour générer les conditions initiales et aux limites de la grille fille.
...
Ces outils sont identifiés comme les \textit{CROCO Pytools} et sont les plus récents intégrés dans l'univers CROCO.
...
\begin{itemize}
    \item création de la grille de calcul (grille CROCO) décrivant la morphologie du domaine modélisé ;
...
Les CROCO Pytools, avec \textit{make\_grid.py}, permettent la préparation de deux types de grilles emboîtées.
La première méthode, AGRIF permet de mettre en place le système d'emboîtement de grilles simultané et rétrospectif su même nom.
La seconde méthode, Offline Zoom, permet de mettre en place manuellement et successivement un emboîtement linéaire de grilles.

La méthode d'emboîtement des grilles AGRIF a été testée dans un premier temps.
Il est apparu que son implémentation dans les CROCO Pytools est encore incomplète.
D'autre part, l'utilisation de AGRIF impose des dépendances sur les clefs autorisées pour la modélisations (définies dans la fichier \textit{cppdefs.h}) (c'est bien le cas ?)

Ainsi, c'est la méthode Offline zoom qui est utilisée pour la suite du rapport.
Cette méthode semble correctement implémentée dans les CROCO Pytools (ajouter qu'on a des doutes du la prise en charge du wet/dry ?)

\subsubsection{La génération des données d'entrée}
\label{generation donnees entrée}
Comme décrit en \ref{subsub:forcages}, chaque grille CROCO, pour être utilisée par le modèle, doit être accompagnée par d'autres fichiers de forçages qui contraignent l'initialisation et la mise en mouvement de la masse d'eau.

Les outils de pré-traitement originaux n'ont été utilisés que pour la génération des conditions initiales et aux limites des grilles.
Les deux fichiers de forçage restants - par la marrée et atmosphériques \ref{par:forcages_atm} - ont été générés à partir de données brutes par deux scripts Python développés dans le cadre du stage.
...
\label{par:cond_init}
Le fichier "croco\_ini.nc" est généré avec le script \textit{make\_ini.py} pour la grille mère et avec le script \textit{make\_zoom\_icb.py} pour toutes les grilles filles.

\textbf{Les conditions aux bordures}\\
\label{par:cond_bords}
Le fichier "croco\_bry.nc" est généré avec le script \textit{make\_bry.py} pour la grille mère et avec le script \textit{make\_zoom\_icb.py} pour toutes les grilles filles, en même temps que les conditions initiales.
...
Dans ce script, les amplitudes et phases des vitesses des ondes de marée sont transformées en ellipse de périodicité selon les formules utilisée dans le code des CROCO Pytool de référence avec la fonction ap2ep. Cette fonction est aussi décrite dans \cite{ap2ep},
