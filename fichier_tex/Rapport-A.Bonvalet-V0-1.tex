...
\end{itemize}
Un détail de ces trois parties est donné ci-dessous.
...
Ce travail a mené à une synthétisation de la marche à suivre par la création d'une figure qui est présentée en Figure \ref{fig:workflow_prepro_original}.
Cette figure est utile pour une prise en main des CROCO Pytools.

[texte sur la figure de synthèse]
...
Parmi les CROCO Pytools, quatre outils ont été utilisés ou mis en \oe{uvre} moyennant des modifications sensibles.
...
Uune nouvelle version du modèle de marée FES (FES2022) est disponible depuis peu mais ne contient pas les données de vitesse de l'onde de marée.
...
La différence majeure entre ces deux modèles réside dans le nombre d'harmoniques de marée qu'ils contiennent.
...
[à compléter avec la figure une fois qu'elle est réalisée]
...
Le modèle FES2014 a été édité en 2014 son intégration dans les CROCO Pytools est récente.
...
La démarche adoptée est de type reverse engeenering, par analyse des résultats issus de l'utilisation des CROCO tools (en Matlab) disponibles en ligne \parencite{croco_files_examples}.
Le code de ce nouvel outil a été développé pour une utilisation générique au regard des différents formats des jeux de données disponibles.
...
En outre, le développement de l'outil a permis de proposer une nouvelle approche d'interpolation en la rendant paramétrable, ce qui permet de l'adapter au jeu de données utilisé.
...
Les données de forçage atmosphérique sont contenues dans un fichier nommé textit{croco\_blk.nc}.
Aucun outils n'est jusqu'alors présent dans les CROCO Pytools pour générer ce fichier.
Un nouvel outil a donc été développé en reprenant la démarche reverse engeenering et l'objectif d'utilisation générique des jeux de données.
la base de ce qui a été réalisé pour \textit{make\_frc.nc}.
Ce nouvel outil a été nommé \textit{make\_blk.py}.

Pour que les processus effectués par ces deux nouveaux codes de créations des fichiers de forçages soient mis en commun et organisés, quatre modules-classes ont été écrits, chacun contient la définition d'une classe du même nom :
\begin{itemize}
...
En effet, l'absence de lissage de la bathymétrie n'est pas supporté par CROCO dans le cas où elle impose la présence de pentes fortes car les variations brutales de la bathymétrie rendent la modélisation instable engendrant souvent des crash.
Ce nouveau pré-traitement est appliqué au domaine MACLoup final immédiatement après sa création avec le CROCO Pytool correspondant.
En plus de permettre la mise en place d'un lissage plus ou moins intense, il permet la modification du trait de côte en le lissant automatiquement et-ou en ajoutant et en retirant des pixels dont les coordonnées sur le maillages sont manuellement entrés.
